import React, { useMemo } from 'react';
import { dayKey, fmtR } from './metrics.js';
import { slug } from './constants.js';

const WD = ['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'];
const MO = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'];

// Lists the trades closed on a given day. `dayKeyStr` is YYYY-MM-DD.
export default function DayTradesModal({ dayKeyStr, trades, onClose }) {
  const list = useMemo(() => {
    if (!dayKeyStr) return [];
    return trades
      .filter((t) => t.fixed_r != null && dayKey(new Date(t.close_time)) === dayKeyStr)
      .sort((a, b) => new Date(a.close_time) - new Date(b.close_time));
  }, [dayKeyStr, trades]);

  if (!dayKeyStr) return null;
  const d = new Date(`${dayKeyStr}T00:00:00`);
  const totalR = list.reduce((a, t) => a + Number(t.fixed_r), 0);
  const title = `${WD[d.getDay()]}, ${d.getDate()} ${MO[d.getMonth()]} ${d.getFullYear()}`;

  return (
    <div className="modal-backdrop" onClick={onClose}>
      <div className="modal day-modal" onClick={(e) => e.stopPropagation()}>
        <header>
          <h2>{title}</h2>
          <button className="x" onClick={onClose}>×</button>
        </header>

        <div className="day-modal-summary">
          <span>{list.length} trade{list.length === 1 ? '' : 's'}</span>
          <span className={totalR > 0 ? 'win' : totalR < 0 ? 'loss' : ''}>{fmtR(totalR)}</span>
        </div>

        {list.length === 0 ? (
          <p className="muted">No trades on this day.</p>
        ) : (
          <table className="day-table">
            <thead><tr><th>Pair</th><th>Setup</th><th>Session</th><th className="num">R</th></tr></thead>
            <tbody>
              {list.map((t) => (
                <tr key={t.id}>
                  <td><span className={`pill pair-${slug(t.symbol_base || t.symbol)}`}>{t.symbol_base || t.symbol}</span></td>
                  <td>{t.setup ? <span className={`pill setup-${slug(t.setup)}`}>{t.setup}</span> : <span className="muted">—</span>}</td>
                  <td>{t.session ? <span className={`pill session-${slug(t.session)}`}>{t.session}</span> : <span className="muted">—</span>}</td>
                  <td className={`num ${t.fixed_r > 0 ? 'cell-win' : t.fixed_r < 0 ? 'cell-loss' : 'cell-be'}`}>{fmtR(t.fixed_r)}</td>
                </tr>
              ))}
            </tbody>
          </table>
        )}
      </div>
    </div>
  );
}
