...
export function GaugeArc({ value, size = 84 }) {
  ...
  return (
    <svg width={size} height={size / 2 + 8} viewBox={`0 0 ${size} ${size / 2 + 8}`}>
      <path d={`M ${sx} ${sy} A ${r} ${r} 0 0 1 ${ex} ${ey}`} fill="none" stroke={chartPalette().track} strokeWidth="6" strokeLinecap="round" />
      <path d={`M ${sx} ${sy} A ${r} ${r} 0 ${v > 0.5 ? 1 : 0} 1 ${vx} ${vy}`} fill="none" stroke={chartPalette().profit} strokeWidth="6" strokeLinecap="round" />
    </svg>
  );
}

// Full circular progress ring (0..1) — used for Profit Factor.
export function Ring({ value, size = 60, color = chartPalette().profit }) {
  const v = Math.max(0, Math.min(1, value || 0));
  const r = size / 2 - 5;
  const c = 2 * Math.PI * r;
  return (
    <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`}>
      <circle cx={size / 2} cy={size / 2} r={r} fill="none" stroke={chartPalette().track} strokeWidth="5" />
      <circle
