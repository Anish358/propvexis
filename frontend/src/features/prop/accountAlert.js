...
/* THE ACCOUNT ALERT — one strip at the top of the account card, seven states, one truth.
 *
 ...
 *   1. SETUP MISMATCH
 *   2. ACCOUNT BREACH
 *   3. the drawdown warning that is CLOSER TO ITS OWN LIMIT (max DD wins a tie)
 *   4. PHASE PASSED
 *   5. TARGET REACHED
 *   6. TARGET NEAR
 *   7. nothing
 *
 * SETUP MISMATCH OUTRANKS EVEN THE BREACH, and that ordering is the point of it rather
 * than an accident of where it was added. Every state below it is a verdict computed
 * from `start_balance`; when the broker says the account does not hold anything like
 * that, the app cannot tell a real breach from an arithmetic one and must say so
 * instead of picking. A wrong baseline breaks the states in BOTH directions -- one set
 * too low manufactures a breach out of an ordinary losing day, one set too high hides a
 * real one -- so ranking it under the breach would leave the loudest banner in the app
 * firing on a number nothing has checked. The meters underneath still draw, from the
 * configured baseline, so nothing is hidden; only the headline changes.
 *
...
export function describeAccount(data) {
  ...
  return {
    ...
    profitTarget: data.profitTarget
      ...
      : null,
    /* THE ENGINE'S OWN VERDICT ON WHETHER ITS INPUTS AGREE (src/domain/prop/prop.js,
     * balanceReconciliation). `null` means there was nothing to compare -- no broker
     * balance, or no configured baseline -- which is NOT the same as "they agree", so
     * only an explicit `ok: false` raises the alert. */
    balanceCheck: data.balanceCheck ?? null,
 ...
 * of these seven clauses IS the priority, so reading the function top to bottom is
...
export function accountAlertState(acct) {
  ...
  // 0. The configured starting balance and the broker disagree by more than the whole
  //    drawdown band, so every verdict below it was computed from a number the account
  //    itself contradicts. See the header note on why this outranks the breach.
  if (acct.balanceCheck && acct.balanceCheck.ok === false) return ALERT.SETUP_MISMATCH;
...
function messageFor(state, acct, m) {
  ...
  switch (state) {
    case ALERT.SETUP_MISMATCH: {
      /* BOTH NUMBERS, BECAUSE THE APP DOES NOT KNOW WHICH ONE IS WRONG. A demo account
       * added from a $25K prop template and a real $25K account whose balance read is
       * stale produce the identical signal here; naming both lets the trader recognise
       * the one that is theirs in a glance. The sentence also says which of the two the
       * meters underneath are using, so the figures below it are not left ambiguous. */
      const c = acct.balanceCheck;
      return `${who} is set up as a ${fmtMoney(c.startBalance)} account but your broker `
        + `reports ${fmtMoney(c.reported)} — the meters below use your setup.`;
    }
    case ALERT.BREACH: {
...
const PRESENTATION = {
   ...
   * account is gone", BANNER_CRITICAL). Painting a setup warning in it would spend the
 ...
 * or null when the account is in none of the seven states and the strip should not render.
 *
