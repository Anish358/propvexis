...
test('primitive components are built on tokens (no raw hex leaking into the layer)', () => {
  // Extract the Phase 1 component block and assert it references tokens, not
  // hardcoded surface/brand hex (warn-amber literals are the only allowed exception).
  const block = css.slice(css.indexOf('Phase 1 — canonical component layer'));
